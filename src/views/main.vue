<template>
  <div v-loading.fullscreen.lock="loading" :element-loading-text="$t('loading')" :class="['aui-wrapper', { 'aui-sidebar--fold': $store.state.sidebarFold }]">
    <template v-if="!loading">
      <main-navbar  @setParentItem="setParentItem" :parentArr="parentArr"  />
      <main-sidebar @setParentArr="setParentArr" :parentItem="parentItem" />
      <div class="aui-content__wrapper" style='margin-left:225;'>
...
export default {
  ...
  methods: {
    ...
    // 获取当前管理员信息
    getUserInfo () {
      return this.$http.get('/admin-api/user/info').then(({ data: res }) => {
        if (res.code !== 200) {
    ...
    // 获取权限
    getPermissions () {
      return this.$http.get('/admin-api/menu/permissions').then(({ data: res }) => {
        if (res.code !== 200) {
          return this.$message.error(res.msg)
        }

<template>
  <div>
	    ...
	    <el-form-item label="站内信标题：" prop="messageTitle">
	      	<el-input  class='artcontent' v-model="dataForm.messageTitle" placeholder="发件人/收件人名称" clearable></el-input>
	    </el-form-item>
	    <el-form-item label="特殊资源：">
		  ...
		  <el-form-item label="收件人/店铺名：" v-else>
		    <em  v-for='item in vipData'>{{item.nickName || item.storeName}}<i class="el-icon-error" @click='actdelete(item.id)'></i></em>
		  </el-form-item>
	    ...
	    <el-form-item >
	    	<div class='artbtn'>
			    ...
			    <el-button class="btn" type="primary" @click="actSubmite('dataForm',false)" :loading="nextloading">发送并添加下一条</el-button>
			    <el-button class="btn" type="primary" @click="actSubmite('dataForm',true)" :loading="sendloading">发送</el-button>
	    	</div>
			</el-form-item>  
	 </el-form>
	  <div v-if='dialogTableVisible'>
	  	<el-dialog title="请选择" :visible.sync="dialogTableVisible" width="60%">
		  	<list v-if='dialogTableVisible' :showdata='vipData' @actdodelete='actdelete' ref="detailCompon" @actclose='actclose' @actVipNameshop='actVipNameshop' @actVipNamevip='actVipNamevip'></list>
			</el-dialog>
	  </div>
	...
	data () {
	    return {
	      ...
	      dataForm: {
				...
				breaddata: ["内容", "站内信管理", "站内信列表","新增"],
	      rules:{
        	messageTitle :[{required: true, message: '请输入发件人/收件人名称', trigger: 'blur'}],
        },
  ...
  methods: {
    ...
    //发送
		actSubmite(dataForm,flag){
            ...
            this.$http.post(addmessage,this.dataForm).then(({ data: res }) => {
            	console.log(res)
		          if (res.code !== 200) {

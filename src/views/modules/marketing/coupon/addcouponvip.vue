<template>
  <div>
        ...
        <el-form-item label="总发行量：" prop="totalNums">
            <el-input v-model.trim.trim="dataForm.totalNums" type="text"  max="1000000" placeholder="1000"  style="width:400px;"></el-input>
        </el-form-item>
        ...
        <el-form-item class="artfromitem" label="使用门槛：" prop="threshold">
        		<div>单笔订单满</div>
            <el-input v-model.trim.trim="dataForm.threshold"  type="text"  max="1000000" placeholder="0"  style="width:400px;"></el-input>
        		<div>元可用（输入“0”为无门槛优惠券）</div>
        </el-form-item>
        <el-form-item label="有效期：" prop="validityDays">
            <el-input v-model.trim.trim="dataForm.validityDays" placeholder="领取后到期天数" maxlength="3" style="width:220px;">
...
//    startsecond: vueFilter.formHMS(Date.now()),  //当前时间
  ...
  components:{
    ...
    'dataForm.totalNums':function(newV,oldV) {
        ...
        'dataForm.validityDays':function(newV,oldV) {
            for(let i=0;i<newV.length;i++){
                // 只能输入数字
                if(!/[0-9]/g.test(newV[i])){
                    this.dataForm.validityDays = newV.replace(newV[i],"")
                }
            }
        },
    },

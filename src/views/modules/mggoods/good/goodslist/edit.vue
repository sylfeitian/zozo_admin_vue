<template>
  <div>
    ...
    <div v-loading="backScanLoading" v-if="backScanLoading" style="min-height:400px"></div>
    <el-form
      ref="dataForm"
      :rules="rules"
    ...
    >
      ...
      <el-form-item label="主品牌：" class="item">
        <template>
          <span
            v-if="dataForm.brands && dataForm.brands.length!=0 && dataForm.brands[0].isMainBrand==0"
          >{{dataForm.brands[0].brandName}}</span>
        </template>
      </el-form-item>
      <el-form-item label="副品牌：" class="item">
        <template>
          <span
            v-if="dataForm.brands && dataForm.brands.length!=0 &&dataForm.brands[0].isMainBrand==1"
          >{{dataForm.brands[0].brandName}}</span>
        </template>
      </el-form-item>
      ...
      <el-form-item label="日本上架状态：" class="item">
        <template>
          <span
            v-if="dataForm.japanShowWeb=='true' || dataForm.japanShowWeb==1|| dataForm.japanShowWeb==true"
          >可售</span>
          <span
            v-else-if="dataForm.japanShowWeb=='false' || dataForm.japanShowWeb==0 || dataForm.japanShowWeb==false"
          >不可售</span>
        </template>
      </el-form-item>
      ...
      <el-form-item label="颜色尺码：">
        ...
        >
          ...
          <el-table-column prop="specId" label="尺码信息" align="center">
            <template slot-scope="scope">
              <div @click="detShowChange(scope.row)" style="color: #2260D2;cursor:pointer;">查看</div>
            </template>
          </el-table-column>
      ...
      <el-form-item label="分类条件：">
        ...
        >
          <el-col :span="12">
            <div class="grid-content">{{item.parentTagName}}</div>
          </el-col>
          <el-col :span="12">
            <div class="grid-content">{{item.nameJp}}</div>
          </el-col>
        </el-row>
          ...
          <div style="display:flex">
            <div style="display:inline-block;" v-loading="uploading">
              ...
              <div
                class="uploadCover"
                v-if="uploading"
                style="position:absolute;top:0;left:0;width: 190px;height: 190px;  z-index: 1000;"
              ></div>
            </div>
            <div class="goodsImg" style="display:flex;margin-left:5px;">
              <div
                v-for="(item,index) in  dataForm.imgs"
                style="position: relative;width:100px;height:100px;margin-right:5px;"
              >
                <img :src="item.sizeOriginal | filterImgUrl" alt style="width:100%;height:100%" />
                <div class="deletaImg" @click="deletaImgFn(index)">X</div>
              </div>
            </div>
          </div>
        </template>
      ...
      <el-form-item label="商品详情：">
        ...
        <quill-editor-img
          ...
          @artmessageContent="artmessageContent"
    ...
    <el-col :span="24">
      <div
        style="position: fixed;bottom: 0;margin: 0 auto;width: 86%;text-align: center;z-index: 999;background-color: #e6e6e6;padding: 10px 0;"
      >
        ...
        <el-button
          v-if="dataForm.showWeb!=1"
          class="btn"
          type="primary"
          @click="saveData(1,'dataForm')"
  ...
  watch: {
    "dataForm.name": function(newV, oldV) {
      ...
      for (let i = 0; i < newV.length; i++) {
        ...
        if (count > 120) {
          //最多输入120个字符
          this.dataForm.name = newV.substr(
            0,
    ...
    "dataForm.madeIn": function(newV, oldV) {
      ...
      for (let i = 0; i < newV.length; i++) {
        ...
        if (count > 20) {
          //最多输入20个字符
          this.dataForm.madeIn = newV.substr(
            0,
            chineseCount / 2 + characterCount - 1
          );
        }
    ...
    "dataForm.material": function(newV, oldV) {
      ...
      for (let i = 0; i < newV.length; i++) {
        ...
        if (count > 300) {
          //最多输入300个字符
          this.dataForm.material = newV.substr(
            0,
            chineseCount / 2 + characterCount - 1
          );
        }
